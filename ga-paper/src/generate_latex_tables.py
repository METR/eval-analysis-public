...
def _get_summary_md_content(family_key: str, task_repo_dir: pathlib.Path) -> str:
    """Get the content of summary.md for a task family."""
    summary_path = task_repo_dir / family_key / "meta" / "summary.md"
...
def _generate_family_table(
    ...
    for family_key, family_info in blueprint_data.get("task_families", {}).items():
        summary = _get_summary_md_content(family_key, task_repo_dir)
...
def main(
    ...
    family_table = _generate_family_table(
        task_repo_dir, difficulty_file, blueprint_file, labels_file
    )
