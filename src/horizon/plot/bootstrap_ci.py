...
from horizon.utils.plots import add_watermark
...
def _load_overlay_results(
    ...
    Returns:
        DataFrame with columns: agent_key, release_date, p_value, has_inspect
    """
...
def _add_overlay_results(
    ...
    for _, row in overlay_df.iterrows():
        marker = "o" if row["has_inspect"] else "x"
        markersize = 8 if row["has_inspect"] else 10
        markeredgewidth = 1 if row["has_inspect"] else 2
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        default=None,
        help="Path to a benchmark_results.yaml file to overlay on the plot",
    )
    ...
    # Add overlay results if provided
    if args.overlay_results is not None:
        overlay_df = _load_overlay_results(
            args.overlay_results,
            success_percent=script_params.get("success_percent", 50),
        )
        ...
        # Get baseline agents that are before the earliest overlay date
        baseline_before_overlay = agent_summaries_for_fitting[
            pd.to_datetime(agent_summaries_for_fitting["release_date"])
            < earliest_overlay_date
        ]
        logger.info(f"Earliest overlay date: {earliest_overlay_date}")
        logger.info(
            f"Baseline agents before overlay: {baseline_before_overlay['agent'].tolist()}"
        )

        # Re-plot those baseline points in blue (on top of the green ones)
        blue_color = "#1f77b4"
        success_col = f"p{script_params.get('success_percent', 50)}"
        for _, row in baseline_before_overlay.iterrows():
            ax = axs[0]
            yerr_low = row[success_col] - row[f"{success_col}q0.025"]
            yerr_high = row[f"{success_col}q0.975"] - row[success_col]
            logger.info(
                f"Plotting blue point for {row['agent']} at {row['release_date']}, p50={row[success_col]:.2f}"
            )
            ax.errorbar(
        ...
        # Add red overlay points
        _add_overlay_results(axs[0], overlay_df)

        # Fit red trendline through overlay + blue baseline points
        # Combine the data
        overlay_for_fit = overlay_df[["release_date", "p_value"]].rename(
            columns={"p_value": success_col}
        )
        baseline_for_fit = baseline_before_overlay[["release_date", success_col]].copy()
        baseline_for_fit["release_date"] = pd.to_datetime(
            baseline_for_fit["release_date"]
        )
        combined_for_fit = pd.concat(
            [baseline_for_fit, overlay_for_fit], ignore_index=True
        )

        logger.info(
            f"Combined data for red trendline fit ({len(combined_for_fit)} points):"
        )
        for _, row in combined_for_fit.iterrows():
            logger.info(f"  {row['release_date']}: {row[success_col]:.2f}")

        if len(combined_for_fit) >= 2:
            red_reg, red_score = fit_trendline(
                combined_for_fit[success_col],
                pd.to_datetime(combined_for_fit["release_date"]),
                log_scale=True,
            )
            red_doubling_time = np.log(2) / red_reg.coef_[0]
            logger.info(
                f"Red trendline doubling time: {red_doubling_time:.1f} days, R²={red_score:.3f}"
            )

            # Plot red trendline
            red_color = "#cc0000"
            red_dashed_outside = (
                combined_for_fit["release_date"].min(),
                combined_for_fit["release_date"].max(),
            )

            red_annotation = plot_trendline(
                ax=axs[0],
                dashed_outside=red_dashed_outside,
                plot_params=plot_params,
                trendline_params={
                    "after_date": script_params["trendlines"][0]["after_date"],
                    "color": red_color,
                    "line_start_date": combined_for_fit["release_date"].min(),
                    "line_end_date": trendline_end_date,
                    "display_r_squared": True,
                    "data_file": None,
                    "styling": None,
            ...
            # Add red annotation above the green one
            if (
                not script_params.get("hide_regression_info", False)
                and red_annotation is not None
            ):
                red_annotation["color"] = red_color
                axs[0].annotate(
                    **red_annotation,
                    xy=(0.98, 0.18),
                    xycoords="axes fraction",
