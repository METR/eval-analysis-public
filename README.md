...
└── reports/
    ├── sonnet-4-5/        # Claude Sonnet 4.5 model report
    │   ├── dvc.yaml       # DVC pipeline definition
    │   ├── params.yaml    # Report parameters
    │   └── data/raw/
    │       └── runs.jsonl # Run data
    └── inspect-transition/ # Inspect transition report
        ├── dvc.yaml
        ├── params.yaml
        └── data/raw/
            └── runs.jsonl
...
```bash
# Run the sonnet-4-5 report
cd reports/sonnet-4-5
dvc repro

# Run the inspect-transition report
cd reports/inspect-transition
dvc repro
```
...
# Load logistic fits
fits = pd.read_csv("reports/sonnet-4-5/data/wrangled/logistic_fits/headline.csv")

# See horizons by agent
print(fits[["agent", "p50", "p50q0.025", "p50q0.975"]].sort_values("p50", ascending=False))

# Load raw runs
runs = pd.read_json("reports/sonnet-4-5/data/raw/runs.jsonl", lines=True)
...
### sonnet-4-5
The main model report with comprehensive analysis of 48+ models, including:
- Time horizon trends (p50, p80)
- Bootstrap confidence intervals
- Token usage analysis

### inspect-transition
Compares results between evaluation frameworks, with the key stage `compare_doubling_times_vs_sonnet` that generates the comparison plot.
